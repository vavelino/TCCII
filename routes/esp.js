  ...
  if (req.params.id == 'ESP_1') {
    if (pilhaPedidoExterno.GetCount() == 0) {
      if (fila.length == 0) {
        ...
        // Caso haja pedido de incoerência
      ...
      case 'L': //    L/LOG/0.TXTa9ff582c$N$10
        ...
        if (mensgrecebida[1] != '$') {
          ...
          usudb.connection.query(sql, function (err, posts, field) {
            // verificar se esse log já foi gravado
            if (posts.length == 0) {
...
function agora() {
  ...
  console.log('Servidor Mandando a Hora certa: ' + Date.now());
  var envio = 't\nt\n' + ag;
...
routeresp.get('/log/:ordem/:pagina', aute, (req, res) => {
  let controleOrdem;
  ...
  switch (controleOrdem) {
    case '0':
      organizado = '0';
      sql =
        ' select	u.nome as nome,    l.id as id,    l.numero as numero,    l.aute as aute,    l.tempo as tempo,    l.dt as dt     from usuario u    right OUTER JOIN  log l	on u.numero = l.numero ORDER BY nome ASC';
      break;
    case '1':
      organizado = '1';
      sql =
        ' select	u.nome as nome,    l.id as id,    l.numero as numero,    l.aute as aute,    l.tempo as tempo,    l.dt as dt     from usuario u    right OUTER JOIN  log l	on u.numero = l.numero ORDER BY aute ASC';
      break;
    case '2':
      organizado = '2';
      sql =
        ' select	u.nome as nome,    l.id as id,    l.numero as numero,    l.aute as aute,    l.tempo as tempo,    l.dt as dt     from usuario u    right OUTER JOIN  log l	on u.numero = l.numero ORDER BY dt ASC';
      break;
    case '3':
      organizado = '3';
      sql =
        ' select	u.nome as nome,    l.id as id,    l.numero as numero,    l.aute as aute,    l.tempo as tempo,    l.dt as dt     from usuario u    right OUTER JOIN  log l	on u.numero = l.numero ORDER BY tempo ASC';
      break;
    case '4':
      organizado = '4';
      sql =
        ' select	u.nome as nome,    l.id as id,    l.numero as numero,    l.aute as aute,    l.tempo as tempo,    l.dt as dt     from usuario u    right OUTER JOIN  log l	on u.numero = l.numero ORDER BY id ASC';
      break;
    default:
      organizado = '4';
      sql =
  ...
  usudb.connection.query(sql, function (err, posts, field) {
    ...
    for (a = 0; a < posts.length; a++) {
      ...
      var dataFormatada =
        'Hora: ' +
        addZero(data.getUTCHours()) +
        ':' +
        addZero(data.getUTCMinutes()) +
        ':' +
        addZero(data.getUTCSeconds()) +
        ' | Data: ' +
        ('0' + data.getDate()).substr(-2) +
        '/' +
        ('0' + (data.getMonth() + 1)).substr(-2) +
...
var vez = 12; // = 0;
routeresp.get('/inco', (req, res) => {
  // Giro de incoerrências
  vez = 0;
  res.redirect('/');
});
...
var Incoerencia = setInterval(function () {
  ...
  switch (vez) {
    ...
    case 6:
      if (tempoIncoerencia != tempo) {
        tempoIncoerencia = tempo;
        fila.unshift('B\na\n1'); // Exclui os arquivos dentro da pasta 1
        vez++;
        n_banco = 0;
      }
      break;
